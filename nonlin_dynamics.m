...
g= 9.807;
    ...
    % Nonlinear dynamics
    dpp = wx + (wz*cos(p)*sin(q))/cos(q) + (wy*sin(p)*sin(q))/cos(q);
...
%      dwz = (trw + Jx*wx*wy - Jy*wx*wy)/Jz;
     ...
     dwx = (Ft*dr*ltvc + Jy*wy*wz - Jz*wy*wz)/Jx;
     dwy = ((Ft*dp*cos(dr))*ltvc - Jx*wx*wz + Jz*wx*wz)/Jy;
     dwz = (trw + Jx*wx*wy - Jy*wx*wy)/Jz;
...
%      dvz = (Ft*cos(dp)*cos(dr) - g*m*cos(p)*cos(q))/m;
     ...
     dvx = (Ft*dr + g*m*sin(q))/m;
     dvy = (Ft*dp*cos(dr) - g*m*cos(q)*sin(p))/m;
     dvz = ((Ft^2*dp^2 + Ft^2*dr^2 + Ft^2)^(1/2) - g*m*cos(p)*cos(q))/m;
    ...
    if( dvz > 0 || pz > 0 )
        ...
        dx = [dpp; dq; dc; dwx; dwy; dwz; dpx; dpy; dpz; dvx; dvy; dvz];
    else
        % Return the derivatives
        dx = [dpp; dq; dc; dwx; dwy; dwz; 0; 0; 0; 0; 0; 0];
    end
